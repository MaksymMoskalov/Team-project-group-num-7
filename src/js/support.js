...
const charityFundsButton = document.querySelector('.icon-slide');

let counter = 0;

charityFundsContainer.insertAdjacentHTML(
  'afterbegin',
  createMarkup(charityFunds)
);
...
function toggleCharityBox() {
  if (charityFundsContainer.classList.contains('charity-box-clicked')) {
    ...
    charityFundsButton.classList.remove('icon-slide-clicked');
  } else {
    charityFundsContainer.classList.add('charity-box-clicked');
    charityFundsButton.classList.add('icon-slide-clicked');
  }
}
