...
function createMarkupBookCard(arr) {
  const bookCardMarkup = arr
    .map(({ _id, book_image, title, author }) => {
      return `<li class="book-card-item"; data-id="${_id}"><img class="book-card" src="${book_image}"  alt="${title} book image"><h3 class="book-card-header">${title}</h3><p class="book-card-text">${author}</p></li>`;
    })
    .join('');
